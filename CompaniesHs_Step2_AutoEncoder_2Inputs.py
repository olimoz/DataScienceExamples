...
def define_and_compile_ac(  input_df,
    ...
    ####################################################################   
    ## Instantiate inputs ##
    # we'll start with the simplest model, 10cols (2+10+10)
    ...
    encoder = Model(inputs  = inputs_encoder, 
                    outputs = latent_space_embeddings, 
                    name    = 'encoder')
    ...
    # split into regression fields and binary fields, separate losses for each
    recons_regres = Lambda(lambda layer: layer[:, 0:12])(reconstruction)
    recons_binary = Lambda(lambda layer: layer[:,12:  ])(reconstruction)

    # instantiate decoder model
    decoder = Model(inputs = inputs_decoder, 
                    output = [recons_regres, recons_binary],
                    name   = 'decoder')
    ...
    # DECODE FROM LATENT SPACE & RECONSTRUCT
    decoder_outputs_list = decoder(latent_space_embeddings)

    # keras now expects the outputs list to be [decoder, decoder]
    # which it has named after the decoder layer
    # in order to use sensible names we use dummy layers:
    outputs_regres_dummy = Lambda(lambda x: x, name='outputs_regres')
    outputs_regres = outputs_regres_dummy(decoder_outputs_list[0])

    outputs_binary_dummy = Lambda(lambda x: x, name='outputs_binary')
    outputs_binary = outputs_binary_dummy(decoder_outputs_list[1])

    # Autoencoder = Encoder + Decoder
    autoencoder = Model(inputs  = inputs_encoder, 
                        outputs = [outputs_regres, outputs_binary],
                        name    = 'autoencoder')
    
    # get two losses and their relative weights
    losses      = {'outputs_regres' : 'mse',
                   'outputs_binary' : 'binary_crossentropy'}
    loss_weights= {'outputs_regres' : 1.0,
                   'outputs_binary' : 1.0}
    ...
    autoencoder.compile(optimizer = 'adadelta', 
                        ...
                        #loss_function(original_dim, has_nan, recon_loss_method)
...
def save_model(model_object, filename, history, save_history=False, proj_root=proj_root):
    ...
    model_object.save(os.path.join(proj_root,'SaveModels_Autoenc','TwoLossModel',filename+'_model.h5'))

    # save weights only (as backup)
    model_object.save_weights(os.path.join(proj_root,'SaveModels_Autoenc','TwoLossModel',filename+'_weights.h5'))

    # save summary text
    filename_txt = os.path.join(proj_root,'SaveModels_Autoenc','TwoLossModel',filename+'_summary.txt')
    with open(filename_txt, 'w') as f:
        with redirect_stdout(f):
            model_object.summary()
    
    # save graph image
    filename_png = os.path.join(proj_root,'SaveModels_Autoenc','TwoLossModel',filename+'_graph.png')
    plot_model(model_object, to_file=os.path.join(proj_root,'SaveModels_Autoenc','TwoLossModel',filename_png), show_shapes=True)
   
    # save training history
    #if save_history:
    filename_history = os.path.join(proj_root,'SaveModels_Autoenc','TwoLossModel',filename+'_history.npy')
...
def grid_search(x_train, x_valid, params):
        ...
        for leaky_alpha in params.get('leaky_alpha'):
            for apply_batchnorm in params.get('apply_batchnorm'):
                for batch_size in params.get('batch_size'):
                    ...
                    history = autoenc.fit(  
                                        x          = x_train,
                                        y          = {"outputs_regres": x_train.iloc[:,0:12], 
                                                      "outputs_binary": x_train.iloc[:,12: ]}, # y=x for autoencoder
                                        epochs     = epochs, 
                                        batch_size = batch_size,
                                        verbose    = 2,
                                        shuffle    = True,
                                        validation_data = (x_valid, {"outputs_regres": x_valid.iloc[:,0:12], 
                                                                     "outputs_binary": x_valid.iloc[:,12: ]}),
    ...
    results.to_csv(os.path.join(proj_root,'SaveModels_Autoenc','TwoLossModel', 'GridScan_Results_'+data_name+'.csv'))
...
def matrix_of_plots(data_name_5char='10col',       # '10col' or 'catego'
                    ...
                    subfolder = ['SaveModels_Autoenc', 'TwoLossModel'],
    ...
    if plot_type == 'histories':
        save_models = [file for file in save_models if file.endswith('autoenc_history.npy') and file.startswith('data_' + data_name_5char)]
        prefix = 'Training histories for '
        xlabel = 'epochs'
        ylabel = 'loss'
    else: # embeddings
        save_models = [file for file in save_models if file.endswith('autoenc_model.h5') and file.startswith('data_' + data_name_5char)]
